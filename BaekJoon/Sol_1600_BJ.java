		...
		while(!q.isEmpty())
		{
			...
			if(temp.k > 0) //말 가능하면
			{
				for(int i = 0; i <8; i ++)
				{
					...
					if(memo[ty][tx][temp.k-1] > (temp.move + 1))
					{
						memo[ty][tx][temp.k-1] = temp.move + 1;
			...
			for(int i = 0; i <4; i ++)
			{
				...
				if(isOver(ty, tx)) continue;
				if(arr[ty][tx] == 1 )continue;
				if(memo[ty][tx][temp.k] > (temp.move + 1))
				{
					memo[ty][tx][temp.k] = temp.move+1;
